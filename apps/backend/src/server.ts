...
app.post('/contagens', async (req, res) => {
	...
	if (!codigo || !responsavelId) {
		return res.status(400).json({ error: 'Campos código e responsavelId são obrigatórios'})
	}
	...
	try {
		...
		if (produtos.length === 0) {
			return res.status(400).json({ error: 'Não há produtos cadastrados para iniciar uma contagem'})
		}
